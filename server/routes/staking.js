...
router.post('/', (req, res) => {
  const { wallet_id, cryptocurrency, amount, apy, lock_period_days } = req.body;
  ...
  const lockPeriod = lock_period_days || 30; // Default 30 days
  
  // Calculate unstake deadline based on lock period
  const unstakeDeadline = new Date();
  unstakeDeadline.setDate(unstakeDeadline.getDate() + lockPeriod);
  
  db.run(
    `INSERT INTO staked_assets (id, wallet_id, cryptocurrency, amount, apy, status, lock_period_days, unstake_deadline) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, wallet_id, cryptocurrency, amount, apyValue, 'active', lockPeriod, unstakeDeadline.toISOString()],
    function(err) {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
      res.status(201).json({
        ...
        staked_asset: { 
          id, wallet_id, cryptocurrency, amount, 
          apy: apyValue, status: 'active', 
          lock_period_days: lockPeriod,
          unstake_deadline: unstakeDeadline.toISOString()
        }
      });
    }
  );
...
// Update unstake deadline
router.put('/:id/deadline', (req, res) => {
  const { unstake_deadline } = req.body;
  
  if (!unstake_deadline) {
    return res.status(400).json({ error: 'Unstake deadline is required' });
  }

  db.run(
    'UPDATE staked_assets SET unstake_deadline = ? WHERE id = ?',
    [unstake_deadline, req.params.id],
    function(err) {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
      ...
      res.json({ message: 'Unstake deadline updated successfully' });
    }
  );
});
